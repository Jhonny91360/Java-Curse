...
public class PersonalInfoTestController {
    ...
    @GetMapping("/{id}")
    public PersonalInfo getPersonalInforById(@PathVariable Long id){
        Optional<PersonalInfo> info = personalInfoService.findById(id);
        if(info.isPresent()){
            return info.get();
        }else{
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,"No se encontro informacion personal con el ID: "+id);
        }
    }
